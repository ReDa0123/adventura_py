...
_goto_first_part = 'Přesunul ses do prostoru:\n'


def _get_take_message(name: str, weight: str | int) -> str:
    """Vrátí zprávu o převzetí předmětu do batohu."""
    return f'Dal sis do batohu (váha {str(weight)}kg) {name}'


def _get_drop_message(name: str) -> str:
...
MISTAKE = Scenario(stMISTAKES, (
    ...
    ScenarioStep(tsBAG_FULL, 'Vezmi lano',  # Zadaný příkaz
                 'Zadaný objekt se už do košíku nevejde: lano',
                 place='smetiště',
                 neighbors=('křižovatka',),
                 items=('lano', 'páčidlo'),
                 bag=("žebřík",),
    ...
    ScenarioStep(tsNOT_IN_BAG, 'polož lano',  # Zadaný příkaz
                 'Zadaný objekt v košíku není: lano',
                 place='smetiště',
                 neighbors=('křižovatka',),
                 items=('lano', 'páčidlo'),
                 bag=("žebřík",),
...
MISTAKE_NS = Scenario(stMISTAKES_NS, (
    ...
    ScenarioStep(tsNS0_WrongCond, 'prozkoumej',  # Zadaný příkaz
                 'V aktuálním prostoru si nic nového nenašel',
                 place='auto',
                 neighbors=("křižovatka",),
                 items=("žebřík",),
                 bag=(),
